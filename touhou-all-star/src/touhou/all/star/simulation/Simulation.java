...
//					enemyShot1();
	...
	public void moveReimuUp (float delta, float scale) {
		...
		if (reimu.position.y > PLAYFIELD_MAX_Y) reimu.position.y = PLAYFIELD_MAX_Y;
	}
	public void moveReimuDown (float delta, float scale) {
		if (reimu.isExploding) return;

		reimu.position.y -= delta * Reimu.reimu_VELOCITY * scale;
		if (reimu.position.y < PLAYFIELD_MIN_Y) reimu.position.y = PLAYFIELD_MIN_Y;
	}
...
//			gl.glPopMatrix();
	...
	public void shot () {
		if ( !reimu.isExploding) {
			reimuShot = new ReimuShot(new Vector2(0,0).add(reimu.position).add(Reimu.center1));
			reimushots.add(reimuShot);
			reimuShot = new ReimuShot(new Vector2(0,0).add(reimu.position).add(Reimu.center2));
			reimushots.add(reimuShot);
