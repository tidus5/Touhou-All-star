...
public class RendererGL20 implements Renderer {
	...
	void renderBackground() {
		...
		spriteBatch.draw(backgroundTexture, 0, 0-400, Gdx.graphics.getWidth(), Gdx.graphics.getHeight()+400, 0, 0, 
				backgroundTexture.getWidth(), backgroundTexture.getHeight(),false, false);

		spriteBatch.end();
	}
	
	void renderReimu(GLCommon gl, Reimu reimu, float delta) {
		if (reimu.dead) {
			return;
		}
		spriteBatch.begin();
	...
	void renderBoss(GLCommon gl, Boss boss) {
		if (boss.isExploding)
			return;
		spriteBatch.begin();
	...
	void renderReimuShots(GLCommon gl, ArrayList<ReimuShot> reimushots) {
		spriteBatch.begin();
	...
	void renderEnemyShots(GLCommon gl, ArrayList<EnemyShot> enemyshots) {
		spriteBatch.begin();
		spriteBatch.enableBlending();
	...
	void updateBossLifeBar(GLCommon gl, Boss boss) {
		// if(bar.progress<100)
		// bar.progress+=0.5;
		// //重新置零
		// if(bar.progress==100)
